...
JNIEXPORT jint JNICALL Java_create_add(JNIEnv *env,jobject thisObj,jstring name,jstring date,jstring time,jint week,jint day,jint notspecs){
	...
	HRESULT hr = CoInitializeEx(NULL, COINIT_MULTITHREADED);
				...
				LPCWSTR wszTaskName;
				wstring temp = wstring(rname.begin(), rname.end());
				wszTaskName = temp.c_str();
				//Execute the program
				wstring wstrExecutablePath = L"C:\\Users\\PRABHAKARAN\\source\\repos\\NotificationApp\\x64\\Debug\\NotificationApp.exe";
				...
				else if (notificationspecs == 3)
				{
					...
					hr = pTrigger->QueryInterface(
						IID_IMonthlyTrigger, (void**)&pMonthlyTrigger);//point to same task
					pTrigger->Release();
					if (FAILED(hr))
					{
